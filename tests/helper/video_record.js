...
module.exports = {
  start: function (client, done) {
    const settings = client.options.desiredCapabilities
    ...
    if (videoSettings && videoSettings.enabled) {
      ...
      client.ffmpeg = require('child_process').execFile(
        'ffmpeg',
        [
          '-video_size',
  ...
  stop: function (client, done) {
    const ffmpeg = client.ffmpeg
    if (ffmpeg) {
